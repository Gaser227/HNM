...
function guardarFormato()// funcion que guarda la solcitud
{
        try {
            ...
            if(comprobarFormato($folio)){
                ...
                if($res){
                    ...
                    guardarIntercambio(null); // funcion que guarda los datos de la seccion solicitud en la tabla entradaSalida
    ...
    function guardarIntercambio($id)//funcion que guarda los datos de la seccion solicitud en la tabla 
    {
        $nominaSuplente = $_POST['nominaSuplente'];
        $nombreSuplente = $_POST['nombreSuplente'];
        $categoriaSuplente = $_POST['categoriaVal'];
        $estatus = $_POST['estatusVal'];
        $adscripcion = $_POST['adscripcionVal'];
        $plaza = $_POST['plazaVal'];
        $turno = $_POST['turnoSuplente'];
        $cambioGuardia = $_POST['cambioGuardiaSuplente'];
        if($id != null){ //Si la id esta llena, se realiza un update
            ...
            $sql = "INSERT INTO intercambioJornada (fechaCambioGuardia, turnoCambio, nominaSuplente, nombreSuplente, categoriaSuplente, tipoPlaza, idFormato) values(?,?,?,?,?,?,?)";
        }
        $statement = Conexion::getConexion()->prepare($sql);
        $res = $statement->execute([$cambioGuardia, $turno, $nominaSuplente, $nombreSuplente, $categoriaSuplente, $plaza, $idFormato]);
        if($res)
    ...
    function consultarFormato(){
         ...
         try {
            ...
            $sql = 'SELECT  formatos.id,formatos.folio, formatos.tipoSolicitud, formatos.estadoSolicitud, formatos.fechaCreacion, formatos.totalFirmas, formatos.idEmpleado, formatos.personaQuienAutoriza ,
                intercambioJornada.fechaCambioGuardia,intercambioJornada.turnoCambio, intercambioJornada.nominaSuplente, intercambioJornada.nombreSuplente, intercambioJornada.categoriaSuplente,intercambioJornada.tipoPlaza,
                empleado.nomina, empleado.nombre,horario.id, horario.turno, adscripcion.nombre, categoria.nombre,categoria.tipoContrato
             FROM formatos 
            INNER JOIN intercambioJornada ON formatos.id = intercambioJornada.idFormato 
